...
运行目录应为 `dks-download-atoms` skill 目录本身。脚本会直接下载 GitHub Release 的 `dks-local.zip` 并解压 `知识库/原子库/`，不使用 GitHub contents API，因此不会因为 contents API rate limit 导致 403。
