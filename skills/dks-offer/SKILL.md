---
name: dks-offer
description: Design Dankoe-style offers for one-person companies with local atom-library grounding; IMA retrieval only when explicitly requested. Use when the user wants to turn a skill, knowledge, service, consulting idea, coaching idea, newsletter, course, template, community, or digital product into a sellable offer with a clear buyer, promise, mechanism, price, delivery scope, and validation plan. Triggers include /dks-offer, 报价设计, 数字产品, 产品化, 变现, offer, landing page, sell my knowledge.
---
...
- If the user needs traffic for the offer, route to `$dks-content`.
- If the user needs delivery leverage, route to `$dks-ai-system`.
- If the user lacks a clear business direction, route to `$dks-roadmap`.

## Reference

Deep reference: 知识库/Skill知识包/dks_offer框架.md
